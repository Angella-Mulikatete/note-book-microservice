  ...
  try {
    ...
    if (!fileUrl || !fileType) {
      ...
      return NextResponse.json(
        { status: 'error', error: 'Missing fileUrl or fileType', textContent: '', chunks: [] },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { status: 'error', error: `Failed to fetch document: ${response.statusText}`, textContent: '', chunks: [] },
        { status: 500 }
      );
    }
    ...
    if (fileType === 'pdf') {
      ...
      return NextResponse.json(
        { status: 'error', error: 'Unsupported file type', textContent: '', chunks: [] },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { status: 'error', error: `Error processing document: ${error.message}`, textContent: '', chunks: [] },
      { status: 500 }
    );
  }
}
